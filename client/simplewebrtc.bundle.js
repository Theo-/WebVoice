...
function SimpleWebRTC(opts) {
    ...
    var config = this.config = {
            url: 'http://localhost:8890', // Production server
            debug: false,
            localVideoEl: '',
            remoteVideosEl: '',
    ...
    this.webrtc.on('text_message_echo', this.handleIncomingTextMessage.bind(this))
...
SimpleWebRTC.prototype.chgRoomPasswd = function (passwd) {
    this.connection.emit('chgpasswd', passwd);
};

SimpleWebRTC.prototype.actRoomPasswd = function (bool) {
    this.connection.emit('actpasswd', bool);
};

SimpleWebRTC.prototype.send_message = function(text_msg) {
    this.connect.emit("text_message", {
        text: text_msg
    });
};

SimpleWebRTC.prototype.handleIncomingTextMessage = function(data) {
    console.log("Message '"+data.text+"' received");
};
...
SimpleWebRTC.prototype.joinRoom = function (name, passwd, cb) {
    var self = this;
    this.roomName = name;
    passwd=(typeof passwd !== 'undefined') ? passwd : '';
    this.connection.emit('join', name, passwd, function (err, roomDescription) {
        if (err) {
            self.emit('error', err);
...
SimpleWebRTC.prototype.startLocalVideo = function () {
    ...
    this.webrtc.startLocalMedia(null, function (err, stream) {
        if (err) {
            alert("You should autorize WebVoice to use your microphone and your camera if you want to use it.");
...
Peer.prototype.handleMessage = function (message) {
    ...
    if (message.type === 'offer') {
        ...
        peer = getPeerWithId(message.from);
        peer.speaking = false;
...
Peer.prototype.end = function () {
    var index = window.peersArray.indexOf(this);
    window.peersArray.splice(index, 1);
